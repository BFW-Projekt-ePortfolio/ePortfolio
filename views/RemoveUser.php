<?php
    ...
    foreach($this->userList as $user){
        $outputString .= "<strong>".$user[0]->getFirstname()." ".$user[0]->getLastname()."</strong> mit den dazugehörigen ".count($user[1]).' Gästen löschen?';
        $outputString .= '<form action="?cmd=RemoveUser" method="post"><button type="submit" value="'. $user[0]->getId() .'" name="deleteUser">löschen!</button></form>';
        $outputString .= '<br>';
    }
?>
